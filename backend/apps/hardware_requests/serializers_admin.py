"""Serializers for the STAFF request surface — review, issue and return.

Split from `serializers.py` when that file crossed the repo's 300-line hard ceiling.
These carry storage locations, asset tags and requester identity, none of which may
appear on the public surface in `serializers_public`.
"""
...
class AdminRequestSerializer(serializers.Serializer):
    ...
    # Verified upstream check-in context for a checked_in submission; empty for
    # every other submission path.
    checkin_purpose = serializers.CharField(read_only=True)
    checkin_project_name = serializers.CharField(read_only=True)
    checkin_verified_at = serializers.DateTimeField(read_only=True, allow_null=True)
